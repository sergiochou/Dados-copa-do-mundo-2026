...
async def fetch_team_stats(team_id: int) -> dict:
    """Busca estatísticas acumuladas do time no torneio, com cache de 5 min e retry em falhas."""
    if not team_id:
        return {}
    ...
    for attempt in range(3):
        async with sem:
            try:
                # Pequena dispersão temporal para requisições concorrentes
                if attempt > 0:
    ...
    # Apenas salva no cache se a requisição de fato teve sucesso
    if success:
...
async def get_stats_ano_2026(team_id: int):
    ...
    async def fetch_safe_stats(ev_id: int):
        try:
            return await get_estatisticas(ev_id)
        except Exception:
            return None
